        ...
        else if (worldIn.canBlockBePlaced(iblock.getBlock(), pos, false, side, (Entity)null, stack))
        {
            ...
            if (worldIn.setBlockState(pos, iblockstate1, 3))
            {
                ...
                if(ViaMCP.getInstance().getVersion() != ViaMCP.PROTOCOL_VERSION)
                {
                    ...
                    mc.theWorld.func_175731_a(pos.add(0.5, 0.5, 0.5), iblock.getBlock().stepSound.getPlaceSound(), (iblock.getBlock().stepSound.getVolume() + 1.0F) / 2.0F, iblock.getBlock().stepSound.getFrequency() * 0.8F, false);
                }
                else
                {
