...
//    }
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            if (useFOVSetting) {
                var4 = this.mc.gameSettings.gammaSetting;
            ...
            if (!zoomActive) {
                if (zoomDist > 1) {
                    //zoomDist = 1;
                    //TODO: optifine zoom
                    zoomDist += ((1 - zoomDist) / (60 * Minecraft.getDebugFPS() * 0.01)) - 0.000001;
                    zoomDist += ((1 - zoomDist) / (60 * Minecraft.getDebugFPS() * 0.01)) - 0.000001;
                    //0.04
                }
            }
...
//                    this.mc.gameSettings.debugCamEnable = true;
                ...
                if (zoomDist < 4) {
                    //zoomDist += 0.01;
                    zoomDist += (((4) - zoomDist) / (120 * Minecraft.getDebugFPS() * 0.005)) + 0.001;
                    //0.000001
                }
...
//                this.mc.gameSettings.debugCamEnable = Config.zoomSmoothCamera;
            ...
            if (var3 instanceof EntityLivingBase && ((EntityLivingBase) var3).getHealth() <= 0.0F) {
                float f1 = (float) ((EntityLivingBase) var3).deathTime + partialTicks;
                var4 /= (1.0F - 500.0F / (f1 + 500.0F)) * 2.0F + 1.0F;
            }

            Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, var3, partialTicks);

            if (block.getMaterial() == Material.water) {
                var4 = var4 * 60.0F / 70.0F;
            }

            return Reflector.ForgeHooksClient_getFOVModifier.exists() ? Reflector.callFloat(Reflector.ForgeHooksClient_getFOVModifier, new Object[]{this, var3, block, Float.valueOf(partialTicks), Float.valueOf(var4)}) : var4;
        }
    }
