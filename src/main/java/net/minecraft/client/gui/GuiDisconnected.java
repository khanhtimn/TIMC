...
import com.teamti.timc.alts.AltLoginThread;
import com.teamti.timc.alts.GuiAltManager;
import com.teamti.timc.main.TIMC;
import com.teamti.timc.util.ServerHook;
...
public class GuiDisconnected extends GuiScreen
{
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        ...
        if (button.id == 2)
        {
        	final Alt randomAlt = TIMC.getAltManager().getAlts().get(new Random().nextInt(TIMC.getAltManager().getAlts().size()));
            final String user2 = randomAlt.getUsername();
            final String pass2 = randomAlt.getPassword();
            TIMC.altmanagergui.currentEmail = user2;
            (TIMC.altmanagergui.loginThread = new AltLoginThread(randomAlt)).start();
        }
        if (button.id == 3)
        {
        	 if (GuiAltManager.selectedAlt != null) {
        		
             TIMC.getAltManager().getAlts().remove(GuiAltManager.selectedAlt);
