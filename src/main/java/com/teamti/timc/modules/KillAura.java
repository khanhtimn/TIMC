...
import com.teamti.timc.util.AimUtil;
...
public class KillAura extends Module {
	...
	public KillAura() {
        super("Kill Aura", Keyboard.KEY_NONE);
	...
	public void autoBlock(EventMotion event)
	  {
		   ...
		   if (event.getType() == EventType2.PRE) {
	            ...
	            if (this.entities.isEmpty()) {
	                for (Object object : TIMC.core.world().loadedEntityList) {
	            ...
	            if (!this.entities.isEmpty()) {
	                ...
	                while (i < this.entities.size()) {
	                    ...
	                    if ((double)e.getDistanceToEntity(TIMC.core.player()) < distance && (double)e.getDistanceToEntity(TIMC.core.player()) < range.getValue()) {
	                        entity = e;
	                        distance = e.getDistanceToEntity(TIMC.core.player());
	                    }
	                    ++i;
	                }
	        	...
	        	if (this.closestE != null && timer.hasTimeElapsed(100, true)) {
	        		...
	        		if(this.autoblock.getValue()){
               		...
               		TIMC.core.attack(this.closestE);
    ...
    public void onPacket(EventPacketSent event){
    	if(attacking){
       	 ...
       	 double 	posX 		= closestE.posX - mc.thePlayer.posX,
   	  				...
   	  				posY 		= (closestE.posY - 3.5 + TIMC.core.findClosestEntity().getEyeHeight() - mc.thePlayer.posY + (mc.thePlayer.getEyeHeight())),
    ...
    public boolean isOnTeam(EntityLivingBase entity){
    	String namewithcolorcode = TIMC.core.player().getDisplayName().getUnformattedText();
    	String colorcode = namewithcolorcode.replace(TIMC.core.player().getName(), "");
    ...
    public boolean isValidHypixel(EntityLivingBase entity) {
    	 return entity != null &&  
    			 ...
    			 (double)entity.getDistanceToEntity(TIMC.core.player()) <= (!TIMC.core.player().canEntityBeSeen(entity) ? 3.0 :
    ...
    public boolean isEntityInFov(EntityLivingBase entity, double angle) {
        double angleDifference = this.getAngleDifference(TIMC.core.player().rotationYaw, AimUtil.getRotations(entity)[0]);
