package com.teamti.timc.tabgui;
...
public class TabGUI {
	...
	public void keyRight(){
		if(viewingCats){
			...
			showModules = true;
				...
				if(TIMC.getModulesInCategory(cats.get(currentCategory)).size() != 0)
				TIMC.getModulesInCategory(cats.get(currentCategory)).get(cats.get(currentCategory).selectedIndex).toggle();
			}
		}
	}
	...
	public void keyDown(){
		if(viewingCats){
			...
			if(cats.get(currentCategory).selectedIndex < TIMC.getModulesInCategory(cats.get(currentCategory)).size()-1){
		...
		if(viewingCats){
			...
			if(currentCategory > 0){
				...
				cats.get(currentCategory).selectedIndex--;	
					...
					cats.get(currentCategory).selectedIndex = TIMC.getModulesInCategory(cats.get(currentCategory)).size()-1;
				}
			}
		}
	}
