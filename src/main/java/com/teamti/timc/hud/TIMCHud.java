...
import com.teamti.timc.event.events.EventKeyPressed;
import com.teamti.timc.event.events.EventRender2D;
import com.teamti.timc.event.events.EventTick;
import com.teamti.timc.main.TCore;
import com.teamti.timc.main.TIMC;
import com.teamti.timc.main.Module;
import com.teamti.timc.modules.ActiveMods;
import com.teamti.timc.tabgui.TabGUI.Cat;
import com.teamti.timc.util.CircleManager;
...
public class TIMCHud extends GuiScreen {
	...
	public void onKey(EventKeyPressed e){
		int x = 5;
		int y = 0 + ( TIMC.getModule("MiniMap").isToggled() ? 75 + 8 : 0);
	...
	public void renderScreen() {
		...
		if(TIMC.getModule("Compass").isToggled())
			renderCompass(sr);
	...
	public void onTick(EventTick e){
		...
		if(TIMC.tabgui.showModules){
			int categoryIndex = TIMC.tabgui.currentCategory;
			Cat category = TIMC.tabgui.cats.get(categoryIndex);
			List<Module> modules = TIMC.getModulesInCategory(category);
	...
	public void renderHud(){
		...
		//this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/Jello.png"));
		//this.drawModalRectWithCustomSizedTexture(0.5f - 1, 2 - 2.5f, 0, 0, 86, 49, 86, 49);

		if(TIMC.getModule("TabGUi").isToggled()){


			this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/TabGUIShadow.png"));
		...
		//TODO: MINIMAP
		if(TIMC.getModule("MiniMap").isToggled()){

			this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/MiniMap.png"));
			this.drawModalRectWithCustomSizedTexture(4.5f, 4.5f, 0, 0, 168/2f, 168/2f, 168/2f, 168/2f);
			//			this.drawModalRectWithCustomSizedTexture(0.5f, 251/2f, 0, 0, 168/2f, 168/2f, 168/2f, 168/2f);
			yOff += 75 + 8;
		}

		//TODO: KeyStrokes
		if(TIMC.getModule("KeyStrokes").isToggled()){


			this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/keystrokes.png"));
		...
		if(TIMC.getModule("TabGUi").isToggled()){
			...
			if(TIMC.tabgui.showModules){
				...
				if(size != 0){
					...
					this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/TabGUISelector.png"));
					this.drawModalRectWithCustomSizedTexture(85, 45+ smoothTrans, 0, 0, 75 + 10, 17.5f, 75, 17.5f);
				}
				int y = 0;
				for(Module m : modules){
					if(y == 0){
						this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/TabGUIShadow2.png"));
			...
			int y = 0;
			for(Cat c : TIMC.tabgui.cats){
				boolean selected = c.equals(TIMC.tabgui.cats.get(TIMC.tabgui.currentCategory));
				if(selected){
					if(!Float.isFinite(TIMC.tabgui.seenTrans)){
						TIMC.tabgui.seenTrans = 0;
					}
					TIMC.tabgui.seenTrans += (((TIMC.tabgui.currentCategory*15)- TIMC.tabgui.seenTrans)/(5*Minecraft.getMinecraft().getDebugFps()*0.04))-0.001;
					TIMC.tabgui.seenTrans += (((TIMC.tabgui.currentCategory*15)- TIMC.tabgui.seenTrans)/(5*Minecraft.getMinecraft().getDebugFps()*0.04))-0.001;
					this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/TabGUISelector.png"));
					this.drawModalRectWithCustomSizedTexture(5, 45 + TIMC.tabgui.seenTrans, 0, 0, 75, 17, 75, 17);
				}
				y++;
			}
			...
			for(Cat c : TIMC.tabgui.cats){
				...
				c.seenTrans += (((selected ? 7 : 0)-c.seenTrans)/(5*Minecraft.getMinecraft().getDebugFps()*0.04))+0.001;
				this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/"+c.location));
	...
	public void renderArraylist(){
		...
		Collections.sort(TIMC.mods, new ModuleComparator());
		for(Module module : TIMC.mods) {
			if(!module.isToggled())
				continue;
			...
			this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/arraylistshadow.png"));
			//GlStateManager.disableBlend();
	...
	{
		return TIMC.core.normalise(this.mc.thePlayer.rotationYaw, -90 + offset, 90 + offset) * 2.0F;
	}
