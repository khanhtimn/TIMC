	...
	public static float[] getBlockRotations(int x, int y, int z)
	  {
	    ...
	    Entity temp = new EntitySnowball(TIMC.core.world());
	    temp.posX = (x + 0.5D);
	    temp.posY = (y + 0.5D);
	    temp.posZ = (z + 0.5D);
	  ...
	  {
	    return new float[] { getYawChangeToEntity(e) + TIMC.core.player().rotationYaw, getPitchChangeToEntity(e) + TIMC.core.player().rotationPitch };
	  }
	 ...
	 /* public static float[] getRotations(Location location)
	  ...
	  public static float[] getRotations(Entity entity)
	  {
	    ...
	    double diffX = entity.posX - TIMC.core.player().posX;
	    double diffZ = entity.posZ - TIMC.core.player().posZ;
	    double diffY;
	    if ((entity instanceof EntityLivingBase))
	    {
	      ...
	      diffY = elb.posY + (
	        ...
	        TIMC.core.player().posY + TIMC.core.player()
	        .getEyeHeight());
	    }
	    else
	    {
	  ...
	  {
	    double deltaX = entity.posX - TIMC.core.player().posX;
	    double deltaZ = entity.posZ - TIMC.core.player().posZ;
	    double yawToEntity;
	    ...
	    return 
	      MathHelper.wrapAngleTo180_float(-(TIMC.core.player().rotationYaw - (float)yawToEntity));
	  }
	  ...
	  public static float getPitchChangeToEntity(Entity entity)
	  {
	    ...
	    double deltaY = entity.posY - 1.6D + entity.getEyeHeight() - 0.4D - 
	      TIMC.core.player().posY;
	    double distanceXZ = MathHelper.sqrt_double(deltaX * deltaX + deltaZ * 
	      deltaZ);
	    
	    double pitchToEntity = -Math.toDegrees(Math.atan(deltaY / distanceXZ));
	    
	    return -MathHelper.wrapAngleTo180_float(TIMC.core.player().rotationPitch -
	      (float)pitchToEntity);
	  }
