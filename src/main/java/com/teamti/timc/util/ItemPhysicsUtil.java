...
public class ItemPhysicsUtil {
    ...
    private static boolean isBlock(ItemStack itm) {
        try {
            if(Block.getBlockFromItem(itm.getItem()).equals("")) {
            }
            return true;
    ...
    public static void doRender(Entity par1Entity, double x, double y, double z, float par8, float par9) {
        rotation = (System.nanoTime() - tick) / 3000000.0D * 1.0D;
        ...
        if (itemstack.getItem() != null) {
            ...
            if (item != null && !Double.isNaN(item.func_174872_o()) &&
...
//          if ((m3 | m1 == (Material.water ? 1 : 0) | m2)) == Material.water ? 1 : 0) | m4 != 0) {
                    if(((m3 | m1 == Material.water ? 1 : 0) | (m4 | m2 == Material.water ? 1 : 0)) != 0) {
            ...
            GL11.glRotatef(item.rotationYaw, 0.0F, 0.2F, 0.0F);
...
//        		if(Block.getBlockFromItem(itemstack.getItem()) == Blocks.beacon) {
...
//        		}
                        ...
                        if(itemstack.getItem() == Items.skull || Block.getBlockFromItem(itemstack.getItem()) == Blocks.skull) {
                            GlStateManager.scale(0.5F, 0.5F, 0.5F);
                        }
//        		if(itemstack.getItem() == Items.skull || Block.getBlockFromItem(itemstack.getItem()) == Blocks.skull) {
//          			GlStateManager.scale(0.5F, 0.5F, 0.5F);
