...
import com.teamti.timc.fake.FakeNetHandlerPlayClient;
import com.teamti.timc.fake.FakeWorld;
import com.teamti.timc.main.TIMC;
import com.teamti.timc.util.BezierCurve;
import com.teamti.timc.util.EntityUtil;
import com.teamti.timc.util.FontUtil;
import com.teamti.timc.util.RenderHelper;
...
public class GuiAltManager extends GuiScreen
{
    ...
    public static void loadAccountList() {
        try {
            TIMC.getAltManager().getAlts().clear();
            final Scanner scanner = new Scanner(new FileReader(AltFile.ALTS_DIR));
            scanner.useDelimiter("\n");
            while (scanner.hasNext()) {
                TIMC.getAltManager().getAlts().add(new Alt(scanner.next().trim().split(":")[0], scanner.next().trim().split(":")[1]));
            }
            scanner.close();
        }
        catch (Exception ex) {}
    ...
    public void drawScreen(int mouseX, int mouseY, float mouseButton) {
        ...
        try {
            ...
            if (this.world != null && this.player != null ) {
                ...
                EntityUtil.drawEntityOnScreen(39 + (int)(1498/2f), (int)(155 + 217/2f), targetHeight, sr.getScaledWidth()/16 - mouseX/8, sr.getScaledHeight()/16 - mouseY/8, (EntityLivingBase)this.player);
        ...
        //1457 603
        drawTexturedRect(30/2f, 41/2f, 218/2f, 35/2f, "altmanagertitle", sr);
        ...
        for(Alt a : TIMC.getAltManager().getAlts()){
            ...
            if(a.loadFade != 0){
                ...
                GlStateManager.color(1, 1, 1, percent);
                GlStateManager.disableAlpha();
                this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/loader.png"));
    ...
    public void init(){
    	try {
            ...
            EntityUtil.updateLightmap(mc, (World)this.world);
            mc.getRenderManager().func_180597_a((World)this.world, mc.fontRendererObj, (Entity)this.player, (Entity)this.player, mc.gameSettings, 0.0f);
        }
        catch (Throwable e) {
            e.printStackTrace();
    ...
    public void initGui() {
        ...
        DiscordMain.getInstance().getDiscordRP().update("Selecting Account", "");
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if(!passwordField.getText().isEmpty() && !usernameField.getText().isEmpty() && this.isMouseHoveringCenteredRect(sr.getScaledWidth()/2f,  sr.getScaledHeight()/2f + 81 - 15, 339/2f - 30 - 20, 25, mouseX, mouseY)){
        	addAltPrompt = false;
        	TIMC.getAltManager().getAlts().add(new Alt(usernameField.getText(), passwordField.getText()));
        	AltFile.save();
        }
    ...
    public void drawTexturedRect(float x, float y, float width, float height, String image, ScaledResolution sr) {
    	this.mc.getTextureManager().bindTexture(new ResourceLocation("TIMC/"+image+".png"));
