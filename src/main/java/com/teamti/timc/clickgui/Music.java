package com.teamti.timc.clickgui;

import com.google.common.collect.Lists;
import com.google.gson.Gson;
import com.teamti.timc.hud.NotificationManager;
import com.teamti.timc.main.TIMC;
import com.teamti.timc.music.image.Resources;
import com.teamti.timc.music.music.Player;
import com.teamti.timc.util.FontUtil;
import com.teamti.timc.util.ScaleUtils;
import com.teamti.timc.util.Stencil;
...
public class Music {
	...
	public Music(float x, float y) {
		...
		List<ChannelObj> channels = Arrays.asList(new ChannelObj("Trap Nation","UCa10nxShhzNrCE1o2ZOPztg"), 
				new ChannelObj("Electro","UCpO0OSNAFLRUpGrNz-bJJHA"),
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton, TIMCGui parent){
		...
		List<String> channels = Arrays.asList("Trap Nation", "ElectroPos�", "Chill Nation", "Vevo", "MrSuicideSheep", "Trap City", "CloudKid");
		...
		if(searchScreen && this.currentSearch != null){
			...
			for(com.youtube.search.Item v : this.currentSearch.getItems()){
				...
				//if(v.getSnippet().getTitle().contains(" - ")){
					if(v.image != null){
	            		if(this.isHoveringCoords(x - 4.5f + 125, y - 4.5f, 275,250, mouseX, mouseY) && this.isHoveringCoords(this.x + 100 + xCount*86.5f + locationX - 77/2f, this.y + 100 + yCount*105f + locationY - 172/2f, 77, 77, TIMC.jgui.mouseX, TIMC.jgui.mouseY)){
	            			Item i = new Item();
	...
	public void onTick(){
		...
		if(searchScreen && this.currentSearch != null){
			...
			for(com.youtube.search.Item v : this.currentSearch.getItems()){
				...
				//if(v.getSnippet().getTitle().contains(" - ")){
					if(v.image != null){
	            		if(this.isHoveringCoords(x - 4.5f + 125, y - 4.5f, 275,250, TIMC.jgui.mouseX, TIMC.jgui.mouseY) && this.isHoveringCoords(this.x + 100 + xCount*86.5f + locationX - 77/2f, this.y + 100 + yCount*105f + locationY - 172/2f, 77, 77, TIMC.jgui.mouseX, TIMC.jgui.mouseY)){
	            			if(v.hover < 1){
	            				v.hover += 0.5;
	            			}
	            		}else{
	...
	public void drawPanel(int mouseX, int mouseY, float opacity, boolean hideShadow, float leftIntro, float rightIntro){
		...
		this.drawTexturedRect(x - 16, y - 16, 423f, 323f, "music background", sr);
		if(searchScreen){
			GlStateManager.color(1, 1, 1, opacity);
			//searchBar.xPosition = x + 121 + 15.5f;
		...
		if(selectedSong != null){
			...
			float percent = smoothTrans(TIMC.jgui.percent, TIMC.jgui.lastPercent);
    		float percent2 = smoothTrans(TIMC.jgui.percent2, TIMC.jgui.lastPercent2);
    		
    		float newScale = Minecraft.getMinecraft().currentScreen == null ? smoothTrans(TIMC.jgui.outro, TIMC.jgui.lastOutro) : percent > 0.98 ? percent : percent2 <= 1 ? percent2 : 1;
		...
		//}
		List<String> channels = Arrays.asList("Trap Nation", "Electro Pos�", "Chill Nation", "VEVO", "MrSuicideSheep", "Trap City", "CloudKid");
		for(int render = 0; render < 2; render++){
			int channelCount = 0;
			for(String c : channels){
				GlStateManager.color(1, 1, 1, 1);
		...
		if(searchScreen && this.currentSearch != null){
					...
					//if(v.getSnippet().getTitle().contains(" - ")){
						...
						if(v.image != null){
		            		...
		            		if(smoothHover != 0){
		            			...
		            			if(toSubtract < 80){
		            				float percent1 = smoothTrans(TIMC.jgui.percent, TIMC.jgui.lastPercent);
		            	    		float percent2 = smoothTrans(TIMC.jgui.percent2, TIMC.jgui.lastPercent2);
		            	    		
		            	    		float newScale = Minecraft.getMinecraft().currentScreen == null ? smoothTrans(TIMC.jgui.outro, TIMC.jgui.lastOutro) : percent1 > 0.98 ? percent1 : percent2 <= 1 ? percent2 : 1;
			            		...
			            		if(Mouse.isButtonDown(0)){
			            			...
			            			Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/ambient play.png"));
			...
			Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/scroll ball.png"));
		    ...
		    if (Mouse.hasWheel() && this.isHoveringCoords(x - 4.5f + 125, y - 4.5f, 275,250, mouseX, mouseY) /*&& locationY > 100*/) {
	            ...
	            final int wheel = TIMC.jgui.mouseWheel;
	            if (wheel < 0) {
		   ...
		   // BlurUtil.blurAreaBoarder(x - 4.5f + 125, y - 4.5f + 0.5f, 275, 32, 60, 0, 1);
		    ...
		    GlStateManager.color(1, 1, 1, 1);
			//FontUtil.jelloFontRegularBig.drawNoBSCenteredString("ElectroPos�", x + 125 + 275/2f - 8, y + 37/2f - 5, white);
		    /*if(scrollTr == 1){
	...
	public void drawTexturedRect(float x, float y, float width, float height, String image, ScaledResolution sr) {
    	Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/"+image+".png"));
