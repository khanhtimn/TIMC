...
public class Panel {
	...
	public void mouseClicked(int mouseX, int mouseY, int mouseButton, TIMCGui parent){
		...
		if(this.isHoveringCoords(x, y, 100, 30, mouseX, mouseY)){
		 ...
		 int count = 0;
	    for(Module m : TIMC.getModulesInCategory(category)){
	    	boolean isHovering = isHoveringCoords(x, y + 25 + count*15 - scroll, 100, 15, mouseX, mouseY) && this.isMouseHoveringRect2(x - 19/2f + 9.5f, y - 19/2f + 9.5f + 25, x - 19/2f - 9.5f + 238/2f, y - 19/2f - 9.5f + 338/2f, TIMC.jgui.mouseX, TIMC.jgui.mouseY);
	    	if(isHovering){
	    		m.toggle();
	    	}
	    	count++;
	...
	public void onTick(){
		...
		int count = 0;
		for(Module m : TIMC.getModulesInCategory(category)){
	    	boolean isHovering = isHoveringCoords(x, y + 25 + count*15 - scroll, 100, 15, TIMC.jgui.mouseX, TIMC.jgui.mouseY) && this.isMouseHoveringRect2(x - 19/2f + 9.5f, y - 19/2f + 9.5f + 25, x - 19/2f - 9.5f + 238/2f, y - 19/2f - 9.5f + 338/2f, TIMC.jgui.mouseX, TIMC.jgui.mouseY);
	...
	public void drawPanel(int mouseX, int mouseY, float opacity, boolean hideShadow, float leftIntro, float rightIntro){
		...
		if(!hideShadow){
		Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/Panel.png"));
