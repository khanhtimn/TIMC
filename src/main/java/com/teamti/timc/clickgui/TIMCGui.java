...
public class TIMCGui extends GuiScreen {

	
	
	public List<com.teamti.timc.clickgui.Panel> panels = Lists.newArrayList();
	public int mouseX;
	public int mouseY;
	
	public Module selectedModule;
	public com.teamti.timc.clickgui.Panel selectedCategory;
	...
	public TIMCGui(){
		...
		panels.add(new com.teamti.timc.clickgui.Panel(x, y, c));
			x += 119 + 5;
		}
		//music = new Music(x, y);
	}
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
    	selectedValue = null;
       for(com.teamti.timc.clickgui.Panel p : panels){
    ...
    public void onTick(EventTick e){
    	if(!(mc.currentScreen instanceof TIMCGui)){
    		lastOutro = outro;
    		if(outro < 1.7){
    			outro += 0.1f;
...
public void drawShadow(float x, float y, float x2, float y2, float width, float height){
	...
	drawTexturedRect(x, y + height, width, 9, "panelbottom", sr);
    	//TOP
  		/*Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/JelloPanel.png"));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    	...
    	for(Value v : selectedModule.getValues()){
    		...
    		if(v instanceof BooleanValue){
    			...
    			Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation(b.getValue() ? "TIMC/checked.png" : "TIMC/unchecked.png"));
    		...
    		if(v instanceof NumberValue){
    			...
    			GlStateManager.color(1, 1, 1, intro);
    			Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/leftslider.png"));
    	    	...
    	    	GL11.glColor4f(1, 1, 1, 1);
    	    	GlStateManager.color(1, 1, 1, intro);
    			Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/sliderright.png"));
    		    ...
    		    if(intro > 0.5f){
    		    Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/sliderhead.png"));
    		    ...
    		    if((hoverEffect > 1) || this.isHoveringCoords((float) (sliderLeft + (52 - 11)*(actualPercent) - 4.5f- 1.5f) + leftOffset, popupTop2 + 25 + 10 + (34/2f)*y - 4.5f, 23/2f, 25/2f, mouseX, mouseY)){
        			...
        			if(intro > 0.5f){
        				...
        				Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/slidershadow.png"));
    		    	 GL11.glTexParameteri(3553, 10240, 9728);
    	                GL11.glTexParameteri(3553, 10241, 9728);
    	...
    	Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("TIMC/PanelOverlay.png"));
